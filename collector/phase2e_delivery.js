...
function renderEmailFiW(issueId) {
  ...
  // Deterministic Markdown → HTML (no LLM)
  let html = md
    .replace(/^# (.*)$/gm, '<h1>$1</h1>')
    .replace(/^### (.*)$/gm, '<h3>$1</h3>')
    .replace(/\*\*(.*?)\*\*/g, '<b>$1</b>')
    ...
    .replace(/\n\n/g, '<br><br>')
    .replace(/\n/g, '<br>');
  html = '<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:20px;line-height:1.6;color:#111">'
       + html
       + '<hr><p style="font-size:12px;color:#666">Foundry/IP Decision Intelligence — <a href="#">Preferences</a> · <a href="#">Unsubscribe</a> · Provenance via DECISION_SIGNALS DS_v0.1</p></div>';
